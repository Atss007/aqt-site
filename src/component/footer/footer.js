...
const contactInfo = [
  ["📍", "Bangalore, India"],
  ["📞", "+91 98765 43210"],
  ["📧", "contact@atlantaquantumtech.com"],
];

export default function Footer() {
  return (
    <footer className="bg-gray-900 text-gray-300 py-12 sm:py-16 border-t border-white/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
          ...
          {/* Links */}
          <div>
            <h3 className="text-lg sm:text-xl font-semibold text-white mb-3 border-b border-blue-500 pb-2 w-max">
              Quick Links
            </h3>
            <ul className="space-y-2 mt-3">
              {quickLinks.map(([label, href]) => (
                <li key={href}>
                  <Link href={href} className="text-sm sm:text-base hover:text-blue-400 transition-colors">
                    {label}
                  </Link>
                </li>
              ))}
          ...
          {/* Contact */}
          <div>
            <h3 className="text-lg sm:text-xl font-semibold text-white mb-3 border-b border-green-400 pb-2 w-max">
              Contact
            </h3>
        ...
        {/* Copyright */}
        <div className="border-t border-gray-700 mt-10 pt-6 text-center text-gray-500 text-xs sm:text-sm">
          © {new Date().getFullYear()} Atlanta Quantum Tech Pvt Ltd. All rights reserved.
        </div>
      </div>
    </footer>
